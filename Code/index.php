<?php
// /index.php — Tracklly landing with Feedback + sticky header + active nav highlight
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .link{
      ...
      border:1px solid transparent; transition:.15s background,.15s color,.15s box-shadow;
    }
    .link:hover{background:rgba(255,255,255,.06);color:#fff}
    /* ACTIVE state like your "Home" button look */
    .link.active{
    ...
    /* Sections */
    .section{padding:48px 20px; scroll-margin-top:calc(var(--nav-h) + 12px);}
    ...
    .price{display:block;background:var(--panel);border:1px solid var(--border);border-radius:var(--radius);padding:18px}
...
<body>
  ...
  <!-- FEEDBACK (own section with big heading OUTSIDE the box) -->
  <section class="section" id="feedback">
    <div class="limiter faq">
      <h2 class="title" style="font-size:28px;margin:0 0 10px 0;">Feedback</h2>

      <div class="qa" style="margin-top:0">
  ...
  <script>
    ...
    const links = new Map(
      [...document.querySelectorAll('.links .link')].map(l => [l.dataset.target, l])
    );
    ...
    // If page loads with hash, mark it active immediately
    if (location.hash) {
